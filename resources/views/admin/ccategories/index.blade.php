                        ...
                        <div class="card-body">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <div class="col-md-6">
                                        <a href="{{ route('ccategories.create') }}">
                                    ...
                                    @foreach ($scategories as $key => $data)
                                        <tr>
                                            ...
                                            <td>{{ $data->sub_cat }}</td>
                                             ...
                                             <td>
                                            @if($data->status == '1')<span class="badge badge-success">{{ "Active"}}</span>@endif
                                            @if($data->status == '0')<span class="badge badge-danger">{{ "InActive"}}</span>@endif

                                            </td>      
                                            
                                          
                                            <td>
                                                    <a href="{{ route('ccategories.edit', [$data->id]) }}"
...
<button data-value="{{ $data->id }}" class="btn btn-danger waves-effect waves-light btn-xs cdelete"><i class="fas fa-trash"></i></button>
                                            ...
                                            <form id="deleteform{{ $data->id }}" method="post" action="{{ route('ccategories.destroy', [$data->id]) }}">
                                                @csrf
                                                {{ method_field('DELETE') }}
                                            </form>
                                            </td>
