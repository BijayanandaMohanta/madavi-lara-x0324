                                ...
                                <table id="example" class="table table-striped table-bordered dt-responsive">
                                    ...
                                    @foreach ($menus as $key => $data)
                                        <tr>
                                            ...
                                            <td>
                                                @if($data->image != '')
                                                    <a href="{{ asset('uploads/menus') }}/{{ $data->image }}" target="_blank">
                                                        <img class="avatar-lg" src="{{ asset('uploads/menus') }}/{{ $data->image }}" alt="{{ $data->heading }}">
                                                    </a>
                                                @else
                                                <span class="badge badge-warning">No Menu Image</span>
                                            @endif
                                            </td>
...
<button data-value="{{ $data->id }}" class="btn btn-danger waves-effect waves-light btn-xs cdelete"><i class="fas fa-trash"></i></button>
                                            ...
                                            <form id="deleteform{{ $data->id }}" method="post" action="{{ route('menus.destroy', [$data->id]) }}">
                                                @csrf
                                                {{ method_field('DELETE') }}
                                            </form>
                                            </td>
