            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form id="form" method="post" action="{{ route('product_image.store') }}"
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <input type="file" class="dropify @error('image') is-invalid @enderror"
                                            ...
                                            data-default-file="{{ asset('uploads/') }}products/{{ $data->image }}"
                                            multiple>
                                        @error('image')
                                        ...
                                        @enderror
                                        <kbd>Upload square image / Recommended 800x800px</kbd>
                        ...
                        <div class="row mt-3">
                                ...
                                <div class="form-group">
                                    ...
                                    @foreach ($product_images as $image)
                                        <div style="position: relative; display:inline-block;">
                                            <img data-src="{{ asset('uploads/products/' . $image->image) }}"
                                                style="width: 10rem;height: 10rem;object-fit: cover;margin-bottom:3px;border-radius: 2px;"
                                                class="lozad">
                                                <select name="priority" id="priority" class="priority" style="position: absolute;bottom: 5px;left: 4px;"  data-image-id="{{ $image->id }}">
                                                    @php
                                                    $a = 1;
                                                    @endphp
                                                    @while ($a <= $product_images_count)
                                                        <option value="{{$a}}"
                                                        @if ($image->priority == $a)
                                                            {{'selected'}}
                                                        @endif
                                            ...
                                            <form method="post"
                                                action="{{ route('product_image.destroy', $image->id) }}">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                            ...
                            <p class="ml-2" id='priority_status'>_</p>
                        </div>
                    </div>
                </div>
            </div> <!-- end col -->
...
<script>
    $(document).ready(function() {
        $('.priority').change(function() {
            ...
            $.ajax({
                url: '{{route('product_image_priority')}}',
                method: 'POST',
                data: {
                    priority: priority,
                    image_id: imageId,
                ...
                success: function(response) {
                    ...
                    $('#priority_status').text('_Priority updated..');
                },
