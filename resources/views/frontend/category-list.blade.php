...
@section('content')
    ...
    <div class="add-section mt-4">
        <div class="container-fluid">
            <div class="row">
                <div class="col-lg-12">
                    <div class="adds-area-sec">
                        <img src="{{ asset("uploads/category/$category->banner_image") }}" alt="{{ $category->category }}">
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                <div class="col-xxl-9 col-xl-9 col-lg-9 order-lg-last col-md-12 order-md-last">
                    ...
                    <div class="catergory-right-sec">
                        <h3 class="blue">{{ $category->category }}</h3>
                        <div class="row align-items-center">
                            @foreach ($products as $index => $product)
                                                ...
                                                <div class="arrival-image">
                                                    <a href="{{ route('product', $product->slug) }}">
                                                        @if ($product->productImages->isNotEmpty())
                                                            <img src="{{ asset('frontend/loader-new.gif') }}"
                                                                data-src="{{ asset("uploads/products/{$product->productImages->first()->image}") }}"
                                                                alt="{{ $product->name }}" class="lozad">
                                                        @else
                                ...
                                @if (($index + 1) % 7 == 0)
                                    <!-- After every 7 products -->
                                    @php
                                        $adIndex = intval($index / 7);
                                    @endphp

                                    @if (isset($categoryads[$adIndex]) && $categoryads->isNotEmpty())
                                        <div class="col-lg-3">
                                            <a href="{{ $categoryads[$adIndex]->redirect_link }}">
                                                <img src="{{ asset("uploads/ads/{$categoryads[$adIndex]->image}") }}"
                                                    alt="" class="img-fluid addproduct">
                                            </a>
                                        </div>
                                    @endif
                                @endif
                            ...
                            <div class="pro-pagination-style text-center mb-30px">
                                <ul>
                                    ...
                                    @if ($products->onFirstPage())
                                        <li><a class="next disabled" href="#"><i class="ion-ios-arrow-left"></i></a></li>
                                    @else
                                        <li><a class="next" href="{{ $products->appends(request()->query())->previousPageUrl() }}"><i class="ion-ios-arrow-left"></i></a></li>
                                    @endif
                                    ...
                                    {{-- Last Page --}}
                                    @if ($last > 1)
                                        <li><a href="{{ $products->appends(request()->query())->url($last) }}">{{ $last }}</a></li>
                                    @endif
                            
                                    {{-- Next Page Link --}}
                                    @if ($products->hasMorePages())
                                        <li><a class="next" href="{{ $products->appends(request()->query())->nextPageUrl() }}"><i class="ion-ios-arrow-right"></i></a></li>
                                    @else
                                        <li><a class="next disabled" href="#"><i class="ion-ios-arrow-right"></i></a></li>
                                    @endif
                                </ul>
                            </div>
                            ...
                            @foreach ($bestsellingproducts as $product)
                                <div>
                                    <div class="feature-slider-item">
                                        ...
                                        <div class="arrival-buttons d-flex justify-content-between">
                                            <a href="addToCart({{ $product->id }},1)" class="add-cart-btn">Add to
                                                cart</a>
                                            <a href="buy_now({{ $product->id }},1)" class="buy-now-btn">Buy
                                                now</a>
                                        </div>
                                        @php
                                            $price = $product->price;
