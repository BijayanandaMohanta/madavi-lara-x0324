...
@section('content')
    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-xxl-12 col-xl-12 col-lg-12">
                <h3 class="subpagetitle">Payment</h3>
            </div>
        </div>
        <div class="row">
            @if (session('success'))
                        ...
                        @forelse ($carts as $cart)
                            <div class="itemdetailsrow">
                                ...
                                <div class="itemremove"><label for=""
                                        class="d-lg-none d-md-none d-block">Remove</label><button type="button"><i
                                            class="fal fa-trash-alt"
                                            onclick='removeCart({{ $cart->id }})'></i></button></div>
                            </div>
                        @empty
                            <div class="itemdetailsrow">
            ...
            <input type="hidden" name="secret_count" value="{{ $carts->count() }}" id="secret_count">

            <div class="col-xxl-4 col-xl-4 col-lg-4" id="cart_sum_table1">
                        ...
                        <form action="" method="POST">
                            ...
                            @if ($grand_total >= 200)
                                <label class="control control--radio">Pay Partial COD
                                    ...
                                    @if($grand_total <= 999)
                                    <br>
                                    <small class="text-danger">Delivery & Handling charges of <b>Rs.99</b> applicable on prepaid orders up to <b>Rs. 399</b> &  <b>COD</b> orders up to <b>Rs.999</b></small>
                                    @endif
    ...
    <script>
        $(document).on("click", "#makepayment", function(e) {
            ...
            const shipping_charges = $('#shipping_charges').data('shippingCharges');
    ...
    <div class="modal fade" id="changeaddressModal" tabindex="-1" aria-labelledby="changeaddressModalLabel"
        aria-hidden="true">
        <form action="{{ route('change_address_payment') }}" method="POST">
            @csrf
                        ...
                        @foreach ($addresses as $address)
                            <div class="p-3">
                                <label class="control control--radio">{{ $address->type }}
                                    ...
                                    <div class="control__indicator"></div>
                                    <p>{{ $address->apartment }}, {{ $address->address }}, {{ $address->city }},
